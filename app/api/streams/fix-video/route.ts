...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            const details: any = await youtubesearchapi.GetVideoDetails(newId);
            const thumbnails = details?.thumbnail?.thumbnails || details?.thumbnails || [];

            if (thumbnails.length > 0) {
                const sorted = [...thumbnails].sort((a: any, b: any) =>
            ...
            console.warn("Fix-video: Failed to fetch thumbnails for replacement video, using defaults.", thumbErr);
        ...
        console.error("Fix-video POST: Unexpected error:", err);
        return NextResponse.json(
            { message: "Failed to apply fixed video", error: err?.message ?? String(err) },
            { status: 500 }
        );
    }
}
