...
export async function POST(req: NextRequest) {
  try {
    ...
    const ms = duration ? (durationMap[duration] ?? null) : null;
    const bannedUntil = type === 'timeout' && ms !== null ? new Date(Date.now() + ms) : null;
    ...
    logger.error({ err: error }, '❌ Stream Ban API Error:');
