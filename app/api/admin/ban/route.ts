...
const AdminBanSchema = z.object({
  ...
  type: z.enum(['ban', 'timeout', 'unban']),
  duration: z.enum(['1min', '1hr', '1d', '1w', '1mo', 'permanent']).optional(),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const callerId = (session?.user as Record<string, unknown>)?.id as string;
    ...
    if ((caller?.platformRole as string) !== 'OWNER') {
      return NextResponse.json({ message: 'Unauthorized' }, { status: 403 });
    }

    const body = await req.json();
    const { targetUserId, type, duration, reason } = AdminBanSchema.parse(body);

    if (targetUserId === callerId) {
      return NextResponse.json(
        { message: 'Owners cannot restrict their own account' },
        { status: 400 },
      );
    }
    ...
    if (type === 'timeout' && duration && duration !== 'permanent') {
      const now = Date.now();
    ...
    logger.error({ err: error }, '❌ Platform Ban API Error:');
