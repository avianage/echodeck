    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500/30">
            <main className="max-w-7xl mx-auto px-6 pt-24 pb-12 space-y-12">
                ...
                <div className={`p-6 rounded-2xl border ${maintenanceActive
                    ? "bg-yellow-500/10 border-yellow-500/30"
                    : "bg-gray-900 border-gray-800"
                } space-y-4`}>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <Wrench className={`w-5 h-5 ${maintenanceActive ? "text-yellow-400" : "text-gray-400"}`} />
                            <div>
                                <h3 className="font-bold text-white">Maintenance Mode</h3>
                                <p className="text-xs text-gray-400">
                                    {maintenanceActive
                                        ? `Active since ${new Date(maintenanceData?.startedAt).toLocaleString()}`
                        ...
                        <div className="flex items-center gap-3">
                            {/* Status badge */}
                            <span className={`text-xs font-bold px-3 py-1 rounded-full ${maintenanceActive
                    ...
                    {maintenanceActive && maintenanceData?.endsAt && (
                        <div className="text-sm text-yellow-300/70">
                            Scheduled to end: {new Date(maintenanceData.endsAt).toLocaleString()}
                        </div>
                    )}
                </div>
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-3xl w-full max-w-md p-6 space-y-6">
                        ...
                        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3">
                            <p className="text-yellow-400 text-xs font-medium">
                                ⚠ All users except you will be immediately redirected to the maintenance page.
                                All active API calls will return 503.
                            </p>
                        </div>
