...
import { Users, Ban, ShieldCheck, ShieldOff, RefreshCw, CheckCircle, XCircle, ShieldAlert, Unlock, Trash2 } from "lucide-react";
...
export function StreamManagement({ creatorId, userRole = "MEMBER", currentUserId }: StreamManagementProps) {
    ...
    const handleApprove = async (viewerId: string, approve: boolean) => {
        try {
            const res = await fetch("/api/streams/access", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ streamerId: creatorId, viewerId, action: approve ? "approve" : "reject" })
            });
            if (res.ok) {
        ...
        <div className="flex flex-col h-full space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-xl font-black tracking-tighter uppercase italic">Stream Management</h3>
                        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Moderation and Access Control</p>
                    </div>
                </div>
                <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => { fetchViewers(); fetchRestrictedUsers(); }}
                ...
                <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 animate-in fade-in">
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-2">
                        {pendingRequests.map((req) => (
                            <div key={req.id} className="flex justify-between items-center gap-3 bg-white/5 px-3 py-2 rounded-xl">
                                <p className="text-xs font-bold text-white truncate max-w-[150px]">@{req.viewer.username || req.viewer.email}</p>
                                <div className="flex gap-1">
                                    <button onClick={() => handleApprove(req.viewer.id, true)} className="p-1.5 hover:bg-green-500/20 text-green-500 rounded-lg transition-colors" title="Approve">
                                        <CheckCircle className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleApprove(req.viewer.id, false)} className="p-1.5 hover:bg-red-500/20 text-red-500 rounded-lg transition-colors" title="Reject">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        ...
                                                        {viewer.displayName && (
                                                            <span className="text-[10px] text-gray-500 truncate">{viewer.displayName}</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className={`flex-shrink-0 px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${
                                                                !user.bannedUntil 
                                                                    ? "bg-red-500/20 text-red-500 border-red-500/30" 
                                                                    : "bg-amber-500/20 text-amber-500 border-amber-500/30"
                                                            }`}>
                                                                {!user.bannedUntil ? "BANNED" : (() => {
                                                        ...
                                                        {user.reason && (
                                                            <span className="text-[10px] text-gray-400 italic truncate max-w-[200px]">"{user.reason}"</span>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
