...
import { Radio, Search, SlidersHorizontal, Music2, Sparkles } from "lucide-react";
...
export default function DiscoverPage() {
    ...
    const handleJoinClick = (stream: any) => {
        if (!session) {
        ...
        router.push(`/party/${stream.user.partyCode || stream.user.id}`);
    };
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-primary/30">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                {userResults.map((user) => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm font-black truncate uppercase tracking-tight">{user.displayName || user.username}</p>
                                            <p className="text-[10px] font-bold text-gray-500 truncate lowercase tracking-wider">@{user.username}</p>
                                        </div>
                                    </button>
                                ))}
                            </div>
                        ) : !searchingUsers && (
                            <p className="text-sm font-bold text-gray-600 italic">No people found matching that name.</p>
                        )}
                    </div>
                )}
            ...
            <GuestJoinModal 
                ...
                callbackUrl={selectedStream ? `/party/${selectedStream.user?.partyCode || selectedStream.user?.id}` : "/discover"}
