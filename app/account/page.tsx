...
export default function AccountPage() {
  ...
  useEffect(() => {
    const checkUsername = async () => {
      if (!isEditingUsername || !debouncedUsername || debouncedUsername === userData?.username)
        return;
  ...
  const handleUpdateUsername = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUserData(userData ? { ...userData, username: newUsername.trim().toLowerCase() } : null);
        setIsEditingUsername(false);
      } else {
  ...
  const handleUpdateDisplayName = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUserData(userData ? { ...userData, displayName: newDisplayName.trim() } : null);
        setIsEditingDisplayName(false);
      } else {
  ...
  const handleDisconnectProvider = async (provider: 'google' | 'spotify') => {
    try {
      ...
      if (res.ok) {
        if (provider === 'spotify') {
          setUserData(userData ? { ...userData, spotifyConnected: false } : null);
        } else {
          setUserData(
            userData
              ? {
                  ...userData,
                  accounts: userData.accounts?.filter(
                    (a: Pick<Account, 'provider'>) => a.provider !== 'google',
                  ),
                }
              : null,
          );
  ...
  const handleUpdateAvatar = async (seed: string | null) => {
    ...
    try {
      ...
      if (res.ok) {
        setUserData(userData ? { ...userData, image: imageUrl } : null);
              ...
              <CardContent className="p-8 space-y-8">
                <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
                  <div className="relative group">
                    ...
                    {user.platformRole && (
                      <div
                        className={`absolute -bottom-2 -right-2 px-2 py-1 rounded-lg text-[8px] font-black border uppercase tracking-widest shadow-xl
                                                ${
                                                  user.platformRole === 'OWNER'
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-4 mt-1">
                        <p className="text-xs text-gray-500 font-medium leading-relaxed">
                          Choose a persona that matches your vibe.
                        </p>
                        {user.image && (
                          <button
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar p-1">
                    {user.platformRole === 'OWNER' && (
                      ...
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src="/avatars/owner_avatar.png"
                          alt="Owner Special"
                          className="w-full h-full object-cover"
                        />
                        <div className="absolute top-0 left-0 right-0 bg-amber-500 py-0.5">
                          <p className="text-[6px] font-black text-white text-center uppercase tracking-tighter">
                            Owner Only
                          </p>
                        </div>
                        {user.image === '/avatars/owner_avatar.png' && (
                ...
                <div className="pt-8 border-t border-white/5 space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div
                      className={`group relative flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl transition-all ${user.accounts?.some((a: Pick<Account, 'provider'>) => a.provider === 'google') ? 'hover:bg-white/[0.04]' : ''}`}
                    >
                      <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-red-400/80">
                      Once you delete your account, all your data including streams, favorites, and
          ...
          <div className="relative w-full max-w-md bg-[#0a0c10] border border-red-500/30 rounded-3xl shadow-2xl p-8 space-y-6">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-gray-400">
                This action is permanent and cannot be undone. You will lose access to all your
                streams and data.
              </p>
            </div>
