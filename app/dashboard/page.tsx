...
import { Play, Users, Globe, Lock, Plus, ArrowLeft, X, Radio, Music } from "lucide-react";
            ...
            <main className="max-w-6xl mx-auto px-6 pt-12 pb-12 space-y-12 relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start w-full">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        {view === "choice" ? (
                            <div className={`grid grid-cols-1 ${(session?.user as any)?.platformRole === "CREATOR" || (session?.user as any)?.platformRole === "OWNER" ? "md:grid-cols-2" : "max-w-md mx-auto"} gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500`}>
                                {/* Start Stream Choice */}
                                {((session?.user as any)?.platformRole === "CREATOR" || (session?.user as any)?.platformRole === "OWNER") && (
                                    <Card 
                            ...
                            <Card className="bg-gray-900/50 border-white/5 backdrop-blur-sm shadow-xl shadow-accent/5 max-w-2xl mx-auto overflow-hidden">
                                ...
                                <CardContent className="p-12 space-y-10">
                                    <div className="space-y-4 text-center">
                                        ...
                                        <p className="text-gray-400 text-sm max-w-sm mx-auto leading-relaxed">Paste the link from <span className="text-accent-400 font-mono">echodeck.avianage.in</span> or enter the unique host code.</p>
                                    </div>
                                ...
                                <div className="py-2 bg-accent/10 border-t border-accent/20 text-center">
                                    <p className="text-[10px] font-black text-accent-400 uppercase tracking-[0.2em] animate-pulse">Waiting for your entry</p>
                                </div>
                            </Card>
                        )}
                    </div>
                    ...
                    <Card className="bg-gray-900/40 border-white/5 backdrop-blur-md sticky top-32 overflow-hidden border-l-0 lg:border-l lg:border-white/5 md:rounded-l-none shadow-2xl">
                            ...
                            {hasFavorites && (
                                <div className="border-b border-white/5">
                                    <div className="px-6 py-4 flex items-center justify-between bg-black/20">
                                        <span className="text-[10px] font-black tracking-[0.2em] text-gray-500 uppercase">Your Circle</span>
                        ...
                        <div className="p-4 bg-white/[0.01] border-t border-white/5 text-center">
                            <p className="text-[9px] font-bold text-gray-600 uppercase tracking-widest">Live Activity Feed</p>
                        </div>
                    </Card>
                </div>
            </main>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="w-full max-w-md bg-gray-900 border border-white/10 rounded-3xl shadow-2xl p-8 space-y-6 animate-in zoom-in-95 duration-300">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={() => setShowSetupModal(false)} className="p-2 rounded-xl bg-white/5 hover:bg-white/10 transition-colors text-gray-400 hover:text-white">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Stream Title</label>
                                <input
                                    value={setupTitle}
                            ...
                            <div>
                                <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Genre <span className="text-gray-600 normal-case font-normal">(optional)</span></label>
                                <input
                                    value={setupGenre}
                            ...
                            <div>
                                <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Visibility</label>
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        ...
                                        className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border font-bold text-sm transition-all ${
                                            setupPublic ? "bg-green-500/10 border-green-500/30 text-green-400" : "bg-white/5 border-white/5 text-gray-500 hover:border-white/10"
                                        }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border font-bold text-sm transition-all ${
                                            !setupPublic ? "bg-amber-500/10 border-amber-500/30 text-amber-400" : "bg-white/5 border-white/5 text-gray-500 hover:border-white/10"
                                        }`}
                                    >
                            ...
                            <div className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/5">
                                ...
                                />
                                <label htmlFor="clearQueue" className="text-xs font-bold text-gray-400 cursor-pointer select-none">
