...
import { User, Shield, Share2, Music, Users, Check, Play, Loader2, UserPlus, ExternalLink } from "lucide-react";
                    ...
                    <div className="relative p-8 rounded-[2.5rem] bg-gradient-to-b from-white/[0.05] to-transparent border border-white/[0.08] backdrop-blur-xl overflow-hidden shadow-2xl">
                        <div className="flex flex-col md:flex-row gap-8 items-center text-center md:text-left">
                            <div className="relative group">
                                ...
                                {user.platformRole && (
                                    <div className={`absolute -bottom-2 -right-2 px-3 py-1.5 rounded-xl text-[10px] font-black border uppercase tracking-widest shadow-2xl backdrop-blur-md
                            ...
                            <div className="space-y-4 flex-1">
                                <div>
                                    ...
                                    <p className="text-xl font-medium text-gray-500 tracking-tight">@{user.username}</p>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                                    {!isSelf && (
                                        <>
                                            {isFriend ? (
                                                <Button disabled className="rounded-2xl bg-green-500/10 text-green-500 border border-green-500/20 font-black uppercase tracking-widest text-[10px]">
                                                    <Check className="w-3.5 h-3.5 mr-2" /> Friends
                                                </Button>
                                            ) : requestPending ? (
                                                <Button disabled className="rounded-2xl bg-gray-500/10 text-gray-500 border border-white/5 font-black uppercase tracking-widest text-[10px]">
                                                    Request Pending
                                                </Button>
                                                ...
                                                >
                                                    {sendingRequest ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-2" /> : <UserPlus className="w-3.5 h-3.5 mr-2" />}
                                                    Add Friend
                                                </Button>
                                            ) : null}
                                        </>
                                    )}
                                    <Button variant="ghost" className="rounded-2xl bg-white/5 hover:bg-white/10 text-[10px] font-black uppercase tracking-widest h-10 border border-white/5">
                            ...
                            <Card className="bg-gray-900/40 border-white/5 backdrop-blur-xl rounded-3xl overflow-hidden shadow-xl">
                                ...
                                <CardContent className="p-6 space-y-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <p className="text-[10px] font-black text-white uppercase tracking-widest">{user.platformRole || "User"}</p>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <p className="text-[10px] font-bold text-gray-500 uppercase">Status</p>
                                        <div className="flex items-center gap-1.5">
                                            <div className={`w-1.5 h-1.5 rounded-full ${user.isLive ? "bg-red-500 animate-pulse" : "bg-gray-500"}`} />
                                            <p className="text-[10px] font-black text-white uppercase tracking-widest">{user.isLive ? "Live" : "Offline"}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <Card className="bg-gradient-to-br from-primary/10 to-transparent border-primary/20 backdrop-blur-xl rounded-[2rem] overflow-hidden shadow-2xl relative">
                                    <CardContent className="p-8 space-y-6">
                                        <div className="space-y-2">
                                            <h3 className="text-2xl font-black italic tracking-tighter uppercase text-primary-400">Creator Pulse</h3>
                                            <p className="text-gray-400 text-sm leading-relaxed max-w-md">
                                                Jamming live and creating vibes. Catch the latest stream or head back to their home party.
                                            </p>
                                        </div>

                                        <div className="flex flex-col gap-4">
                                            <div className="p-4 bg-white/5 border border-white/10 rounded-2xl space-y-2">
                                                <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Public Stream Link</p>
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <Button 
                                                        ...
                                                        onClick={() => {
                                                            navigator.clipboard.writeText(`https://echodeck.avianage.in/party/${user.partyCode || user.id}`);
                            ...
                            <div className="md:col-span-2">
                                <Card className="bg-white/[0.02] border-white/5 rounded-[2rem] p-12 flex flex-col items-center justify-center text-center space-y-4">
                                    ...
                                    <p className="text-sm text-gray-500 max-w-sm">
                                        This user is currently a listener in the EchoDeck social circle. 
                                        Connect with them to see what they are vibing to!
                                    </p>
                                </Card>
                            </div>
                        )}
